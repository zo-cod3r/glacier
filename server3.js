...
const db = new sqlite3.Database('./glacier.db', (err) => {
    if (err) {
        ...
        db.run("CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT UNIQUE, password TEXT, firstName TEXT, middleInitial TEXT, lastName TEXT, email TEXT, phone TEXT, unit TEXT, role TEXT, is_admin INTEGER DEFAULT 0, admin_justification TEXT, comm_vessels TEXT)");
...
app.put('/admin/requests/:id', (req, res) => {
    const isAdminStatus = req.body.action === 'approve' ? 1 : 0;
    db.run("UPDATE users SET is_admin=?, admin_justification='' WHERE id=?", [isAdminStatus, req.params.id], (err) => {
        if (err) return res.status(500).json({ success: false });
        res.json({ success: true });
    });
});
...
app.post('/problems', (req, res) => {
    const ts = (new Date().getMonth()+1).toString().padStart(2,'0') + "/" + new Date().getDate().toString().padStart(2,'0') + "/" + new Date().getFullYear().toString().slice(-2) + " " + new Date().getHours().toString().padStart(2,'0') + ":" + new Date().getMinutes().toString().padStart(2,'0');
    db.run("INSERT INTO problems (submitter, description, timestamp, response_unread) VALUES (?,?,?,0)", [req.body.submitter, req.body.description, ts], err => {
        if (err) return res.status(500).json({ success: false });
        res.json({ success: true });
    });
});
...
app.post('/delays', (req, res) => {
    const ts = (new Date().getMonth()+1).toString().padStart(2,'0') + "/" + new Date().getDate().toString().padStart(2,'0') + "/" + new Date().getFullYear().toString().slice(-2) + " " + new Date().getHours().toString().padStart(2,'0') + ":" + new Date().getMinutes().toString().padStart(2,'0');
    db.run("INSERT INTO delays (aor, vessels, start_date, misle, created_by, created_at, status) VALUES (?,?,?,?,?,?,'Active')", [req.body.aor, req.body.vessels, req.body.startDate, req.body.misle, req.body.createdBy, ts], err => {
        if(err) return res.status(500).json({success:false});
        res.json({success:true});
    });
});
