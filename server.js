...
const db = new sqlite3.Database('./glacier.db', (err) => {
    if (err) {
        ...
        // --- NEW: POC HOURS TABLE ---
        db.run(`CREATE TABLE IF NOT EXISTS poc_hours (
            id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, operation TEXT, 
            cutter TEXT, date TEXT, location TEXT, hour_type TEXT, hours REAL
        )`);
    }
});
...
app.get('/commercial-vessels', (req, res) => {
    db.all("SELECT name FROM commercial_vessels ORDER BY name ASC", [], (err, rows) => res.json({ success: !err, vessels: rows }));
});
...
app.post('/import-vessels', (req, res) => {
    ...
    db.serialize(() => {
        ...
        let addedCount = 0;
        vessels.forEach(v => { if (v.name) { stmt.run(v.name, v.flag, v.type); addedCount++; } });
        stmt.finalize();
...
// --- NEW: POC HOURS ROUTES ---
app.post('/poc-hours', (req, res) => {
    const { entries } = req.body;
    const stmt = db.prepare("INSERT INTO poc_hours (username, operation, cutter, date, location, hour_type, hours) VALUES (?, ?, ?, ?, ?, ?, ?)");
    entries.forEach(e => {
        stmt.run(e.username, e.operation, e.cutter, e.date, e.location, e.hourType, e.hours);
    });
    stmt.finalize();
    res.json({ success: true });
});

app.get('/poc-hours', (req, res) => {
    db.all("SELECT * FROM poc_hours", [], (err, rows) => {
